...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: "zdgaphmmjwokovhy" // app password
  }
});
...
app.post("/signup", upload.single("image"), async (req, res) => {
  try {
    ...
    // Send only safe info in response
    res.json({ message: "Signup successful", user: { name, email, phone, location, skill, image: imagePath } });
...
app.post("/login", async (req, res) => {
  try {
    ...
    res.json({ message: "Login successful", user: { name: user.name, email: user.email, phone: user.phone, location: user.location, skill: user.skill, image: user.image } });
...
app.post("/forgot-password", async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<h2>SkillConnect Password Reset</h2>
...
app.post("/reset-password", async (req, res) => {
  try {
    ...
    if (user.resetCode !== code || Date.now() > user.resetCodeExpiry.getTime()) {
